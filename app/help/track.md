...
Two objects that pass close together can still exchange identities: distance and overlap alone cannot tell which is which at the moment they touch. Nothing here models velocity, and divisions are not detected — a daughter starts a new track. For those, run a tracker that carries a motion model through the Python worker.
